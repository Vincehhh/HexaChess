...
public class PuzzleDAO extends DAO<Puzzle> {
    ...
    public Puzzle create(Puzzle obj) {
        String requete = "INSERT INTO puzzles (puzzle_id, moves, solutions, rating, theme, created_at) VALUES(?, ?, ?, ?, ?, ?)";
        try {
    ...
    public Puzzle update(Puzzle obj) {
        String requete = "UPDATE puzzles SET moves = ?, solutions = ?, rating = ?, theme = ? WHERE puzzle_id = ?";
        try {
            PreparedStatement pstmt = connect.prepareStatement(requete);
