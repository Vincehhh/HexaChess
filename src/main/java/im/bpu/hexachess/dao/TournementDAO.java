...
public class TournamentDAO extends DAO<Tournament> {
    ...
    public Tournament create(Tournament obj) {
        String requete = "INSERT INTO tournaments (tournament_id, name, description, start_time, end_time, winner_id) VALUES(?, ?, ?, ?, ?, ?)";
        try {
    ...
    public Tournament update(Tournament obj) {
        String requete = "UPDATE tournaments SET name = ?, description = ?, start_time = ?, end_time = ?, winner_id = ? WHERE tournament_id = ?";
        try {
            PreparedStatement pstmt = connect.prepareStatement(requete);
